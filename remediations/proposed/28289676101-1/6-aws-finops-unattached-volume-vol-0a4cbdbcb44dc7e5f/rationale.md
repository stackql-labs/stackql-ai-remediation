...
(a) **What/where:** This removes the unattached EBS volume `vol-0a4cbdbcb44dc7e5f` (1 GiB, `gp2`) in region `ap-southeast-2`. The `DELETE` targets `aws.ec2.volumes` and is gated by a preflight query that re-confirms the volume still exists and reports `status = 'available'` (i.e. detached) immediately before deletion.
