# Remediation rationale — active EC2 instance `i-038ae06cdc98be7d9` (zero-VMs policy)

**(a) What is being removed and where.** This finding flags EC2 instance `i-038ae06cdc98be7d9` (type `t3a.xlarge`, AZ `ap-southeast-2a`) in region `ap-southeast-2` as a violation of the zero-VMs policy, with the intended action being to **terminate the instance**. Note the upstream `suggested_remediation.type` is `manual`: **no `preflight_query` or `sql_query` was provided**, so `preflight.sql` and `remediation.sql` in this directory are intentionally empty. The upstream description supplies only a templated, un-substituted stackql hint (`DELETE FROM aws.ec2_native.instances WHERE ... data__Identifier = '<instanceId>'`).
