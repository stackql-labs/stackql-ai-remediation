# Rationale — Unattached EBS volumes (vol-022ccd1d2be1787d6)

(a) **What/where:** This removes the unattached EBS volume `vol-022ccd1d2be1787d6` (1 GiB, `gp2`) in region `ap-southeast-2`. The `DELETE` targets `aws.ec2.volumes` and is gated by a preflight query that re-confirms the volume still exists and is `status = 'available'` (i.e. detached) immediately before deletion.
