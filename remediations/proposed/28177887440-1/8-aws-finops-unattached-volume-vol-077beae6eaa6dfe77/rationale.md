# Rationale — Unattached EBS volumes (vol-077beae6eaa6dfe77)

(a) **What/where:** This removes the unattached EBS volume `vol-077beae6eaa6dfe77` (1 GiB, `gp2`) in region `ap-southeast-2`. The `DELETE` targets `aws.ec2.volumes` and is gated by a preflight query that re-confirms the volume still exists and is `status = 'available'` (i.e. detached) immediately before deletion.
