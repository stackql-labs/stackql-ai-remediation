# Remediation rationale — finding 8: vol-077beae6eaa6dfe77

This change deletes the unattached EBS volume `vol-077beae6eaa6dfe77` (gp2, 1 GiB) in region `ap-southeast-2`. The preflight re-confirms the volume still exists and is in `available` (unattached) state before the `DELETE` is issued against `aws.ec2.volumes`.
