# Remediation rationale — finding 1: vol-0ab197e934110fcc9

This change deletes the unattached EBS volume `vol-0ab197e934110fcc9` (gp2, 1 GiB) in region `ap-southeast-2`. The preflight re-confirms the volume still exists and is in `available` (unattached) state before the `DELETE` is issued against `aws.ec2.volumes`.
