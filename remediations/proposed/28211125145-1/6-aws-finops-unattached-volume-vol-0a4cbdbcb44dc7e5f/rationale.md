# Remediation rationale — finding 6: vol-0a4cbdbcb44dc7e5f

This change deletes the unattached EBS volume `vol-0a4cbdbcb44dc7e5f` (gp2, 1 GiB) in region `ap-southeast-2`. The preflight re-confirms the volume still exists and is in `available` (unattached) state before the `DELETE` is issued against `aws.ec2.volumes`.
