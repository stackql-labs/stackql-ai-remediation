# Remediation rationale — finding 5: vol-0587b977863c0dead

This change deletes the unattached EBS volume `vol-0587b977863c0dead` (gp2, 1 GiB) in region `ap-southeast-2`. The preflight re-confirms the volume still exists and is in `available` (unattached) state before the `DELETE` is issued against `aws.ec2.volumes`.
