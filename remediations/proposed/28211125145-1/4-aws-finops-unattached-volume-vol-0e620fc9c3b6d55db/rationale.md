# Remediation rationale — finding 4: vol-0e620fc9c3b6d55db

This change deletes the unattached EBS volume `vol-0e620fc9c3b6d55db` (gp2, 1 GiB) in region `ap-southeast-2`. The preflight re-confirms the volume still exists and is in `available` (unattached) state before the `DELETE` is issued against `aws.ec2.volumes`.
