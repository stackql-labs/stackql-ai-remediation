# Remediation rationale — finding 10: vol-022ccd1d2be1787d6

This change deletes the unattached EBS volume `vol-022ccd1d2be1787d6` (gp2, 1 GiB) in region `ap-southeast-2`. The preflight re-confirms the volume still exists and is in `available` (unattached) state before the `DELETE` is issued against `aws.ec2.volumes`.
