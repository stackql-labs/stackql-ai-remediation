# Remediation rationale — finding 0: vol-003cc7ab19db52a16

This change deletes the unattached EBS volume `vol-003cc7ab19db52a16` (gp2, 1 GiB) in region `ap-southeast-2`. The preflight re-confirms the volume still exists and is in `available` (unattached) state before the `DELETE` is issued against `aws.ec2.volumes`.
